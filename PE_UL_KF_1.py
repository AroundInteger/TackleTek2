...
class LandmarkTracker:
    ...
    def interpolate_missing_landmarks(self, player_id):
        ...
        for landmark_idx in range(self.num_landmarks):
            ...
            current_ts_idx = 0
            for frame_idx in range(self.impact_frame + 1):
                ...
                if frame_idx not in interpolated_data:
                    interpolated_data[frame_idx] = np.zeros((self.num_landmarks, 2))
                interpolated_data[frame_idx][landmark_idx] = kf.x[:2]
        
        return interpolated_data

    def plot_landmark_trajectories(self, player_id, landmark_idx):
        """Plot original vs filtered trajectories for a specific landmark"""
        ...
        if not original_data:
            logging.warning(f"No data available for {player_id} landmark {landmark_idx}")
            return
        ...
        filtered_frames = np.arange(self.impact_frame + 1)
        filtered_data = np.array([interpolated[f][landmark_idx] for f in filtered_frames])
        ...
        ax1.plot(filtered_frames, filtered_data[:, 0], 'r-', label='Filtered')
        ...
        ax2.plot(filtered_frames, filtered_data[:, 1], 'r-', label='Filtered')
...
def main():
    ...
    with open(csv_filename, 'w', newline='') as csvfile:
        ...
        if impact_detected:
            ...
            # Ball carrier's wrists
            tracker.plot_landmark_trajectories('ball_carrier', 9)  # Left wrist
            tracker.plot_landmark_trajectories('ball_carrier', 10)  # Right wrist
            
            # Tackler's head and shoulders
            tracker.plot_landmark_trajectories('tackler', 0)  # Head
            tracker.plot_landmark_trajectories('tackler', 5)  # Left shoulder
