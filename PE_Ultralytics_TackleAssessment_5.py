...
def identify_players(keypoints, prev_ball_carrier, prev_tackler):
    ...
    if prev_ball_carrier is None or prev_tackler is None:
        # Initial identification: assume leftmost player is ball carrier
        sorted_poses = sorted(poses, key=lambda x: x[0, 0])  # Sort by x-coordinate of first keypoint
        return sorted_poses[0], sorted_poses[1]
    else:
        # Calculate distances between current poses and previous positions
        distances_to_ball_carrier = [np.linalg.norm(pose[0] - prev_ball_carrier[0]) for pose in poses]
        distances_to_tackler = [np.linalg.norm(pose[0] - prev_tackler[0]) for pose in poses]

        # Identify players based on minimum distance to previous positions
        ball_carrier_idx = np.argmin(distances_to_ball_carrier)
        tackler_idx = np.argmin(distances_to_tackler)

        # Ensure we don't assign the same pose to both players
        if ball_carrier_idx == tackler_idx:
            # If conflict, assign based on relative positions
            if poses[ball_carrier_idx][0, 0] < poses[1 - ball_carrier_idx][0, 0]:
                ball_carrier_idx, tackler_idx = ball_carrier_idx, 1 - ball_carrier_idx
            else:
                ball_carrier_idx, tackler_idx = 1 - ball_carrier_idx, ball_carrier_idx
