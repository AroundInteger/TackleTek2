...
def identify_players(keypoints, prev_tackler, prev_ball_carrier):
    ...
    if len(poses) == 2:
        ...
        return left_player, right_player  # tackler, ball_carrier
    elif len(poses) == 1:
        ...
        if dist_to_tackler < dist_to_ball_carrier:
            return pose, prev_ball_carrier
        else:
            return prev_tackler, pose
    else:
        # If we don't have any poses, return the previous positions
        return prev_tackler, prev_ball_carrier
...
def draw_annotations(frame, tackler, ball_carrier, tackler_history, ball_carrier_history):
...
def detect_impact(tackler, ball_carrier):
    ...
    return tackler_shoulder_x - ball_carrier_wrist_x >= 0

# Open the video file
#video_path = "TackleTEK Video Quality Test R1.mp4"
video_path = "R1_1.mp4"
...
for codec, ext in codecs:
    output_path = f'analyzed_rugby_tackle_R1{ext}'
...
# Prepare CSV file
csv_filename = 'tackle_analysis_R1.csv'
csv_header = ['frame', 'tackler_speed', 'ball_carrier_speed', 'shoulder_wrist_height_difference', 'tackler_shoulder_height', 'ball_carrier_waist_height']

tackler_positions = []
ball_carrier_positions = []
tackler_history = deque(maxlen=30)  # Adjust maxlen to change the length of the displayed history
...
try:
    with open(csv_filename, 'w', newline='') as csvfile:
        ...
        while cap.isOpened() and not impact_detected:
            ...
            results = model(frame)
            annotated_frame = results[0].plot()  # Use Ultralytics pose estimation

            if len(results[0].keypoints) >= 1:
                if prev_tackler is None or prev_ball_carrier is None:
                    # Initialize players for the first frame with detections
                    tackler, ball_carrier = initialize_players(results[0].keypoints)
                else:
                    # Use simplified identification for subsequent frames
                    tackler, ball_carrier = identify_players(results[0].keypoints, prev_tackler, prev_ball_carrier)

                if tackler is not None and ball_carrier is not None:
                    prev_tackler, prev_ball_carrier = tackler, ball_carrier

                    tackler_positions.append(tackler[11])  # Using left hip point (index 11)
                    ball_carrier_positions.append(ball_carrier[11])
                    
                    tackler_history.append(tackler)
                    ball_carrier_history.append(ball_carrier)
                    
                    if len(tackler_positions) > 2:
                    ...
                    height_difference, shoulder_height, waist_height = draw_annotations(
                        annotated_frame, tackler, ball_carrier, tackler_history, ball_carrier_history)

                    # Write data to CSV
                    csv_writer.writerow([frame_count, tackler_speed, ball_carrier_speed, height_difference, shoulder_height, waist_height])

                    # Check for impact
                    impact_detected = detect_impact(tackler, ball_carrier)
        ...
        if impact_detected and prev_tackler is not None and prev_ball_carrier is not None:
            final_frame = annotated_frame.copy()
            draw_annotations(final_frame, prev_tackler, prev_ball_carrier, tackler_history, ball_carrier_history)
...
logging.info(f"Tackle analysis data saved to {csv_filename}")