...
def create_debug_view(frame, fg_mask, bg_model, grid_points, h_lines, v_lines, stable_markers):
    ...
    # Create a copy for intersection overlay
    mask_with_intersections = mask_colored.copy()
    
    # Directly overlay red color on intersections
    mask_with_intersections[intersection_mask > 0] = [0, 0, 255]  # Bright red for intersections
    ...
    # Draw grid on original and background panels
    frame_with_grid = draw_masked_grid(frame.copy(), grid_points, h_lines, v_lines, stable_markers, np.zeros_like(fg_mask))
    bg_with_grid = draw_masked_grid(bg_display, grid_points, h_lines, v_lines, stable_markers, np.zeros_like(fg_mask))
    
    # Draw grid on intersection visualization
    mask_with_grid = draw_masked_grid(mask_with_intersections, grid_points, h_lines, v_lines, stable_markers, np.zeros_like(fg_mask))
    ...
    debug_view = np.vstack([
        ...
        mask_with_grid      # Foreground mask with grid and intersections on bottom
    ])
...
def draw_masked_grid(frame, grid_points, h_lines, v_lines, corners, fg_mask):
    ...
    # Create a separate mask for the grid
    grid_mask = np.zeros_like(fg_mask)
    
    # Draw horizontal grid lines into the mask
    for line in h_lines:
        points = np.array(line, dtype=np.int32)
        points = points.reshape((-1, 1, 2))
        cv2.polylines(grid_mask, [points], False, 255, 1)
    
    # Draw vertical grid lines into the mask
    for line in v_lines:
        points = np.array(line, dtype=np.int32)
        points = points.reshape((-1, 1, 2))
        cv2.polylines(grid_mask, [points], False, 255, 1)
    
    # Combine grid mask with background mask
    valid_grid = cv2.bitwise_and(grid_mask, bg_mask)
    
    # Draw the valid grid lines onto the frame
    grid_frame[valid_grid > 0] = [100, 100, 100]
    
    # Draw grid intersection points only in background areas
    for point in grid_points:
        x, y = map(int, point)
        if 0 <= y < bg_mask.shape[0] and 0 <= x < bg_mask.shape[1]:
            if bg_mask[y, x] > 0:  # Only draw if point is in background
