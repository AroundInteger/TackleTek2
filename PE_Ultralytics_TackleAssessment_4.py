...
def analyze_tackle(ball_carrier, tackler):
    ...
    head_below_wrists = tackler[0][1] > min(ball_carrier[9][1], ball_carrier[10][1])
...
def draw_labels_and_indicators(frame, ball_carrier, tackler, head_below_wrists):
    ...
    # Draw circles for ball carrier's wrists (keypoints 9 and 10)
    wrist_color = (0, 255, 255)  # Yellow
    cv2.circle(frame, (int(ball_carrier[9][0]), int(ball_carrier[9][1])), 5, wrist_color, -1)
    cv2.circle(frame, (int(ball_carrier[10][0]), int(ball_carrier[10][1])), 5, wrist_color, -1)
    
    # Draw circle for tackler's head (keypoint 0)
    head_color = (255, 0, 0)  # Blue
    cv2.circle(frame, (int(tackler[0][0]), int(tackler[0][1])), 5, head_color, -1)
    
    # Draw line for ball carrier's wrist height
    wrist_height = min(ball_carrier[9][1], ball_carrier[10][1])
    cv2.line(frame, (0, int(wrist_height)), (frame.shape[1], int(wrist_height)), wrist_color, 1)
    
    # Indicate if tackler's head is below ball carrier's wrists
    indicator_color = (0, 255, 0) if head_below_wrists else (0, 0, 255)
    cv2.circle(frame, (int(tackler[0][0]), int(tackler[0][1])), 10, indicator_color, 2)

    # Add annotations for wrist and head heights
    height, width = frame.shape[:2]
    annotation_y = height // 2
    cv2.putText(frame, f"Ball Carrier Wrist Height: {int(wrist_height)}", (10, annotation_y), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, wrist_color, 2)
    cv2.putText(frame, f"Tackler Head Height: {int(tackler[0][1])}", (10, annotation_y + 30), 
...
csv_header = ['frame', 'ball_carrier_speed', 'tackler_speed', 'impact_detected', 'head_below_wrists', 'head_back_hips_inline', 'ball_carrier_wrist_height', 'tackler_head_height']
...
with open(csv_filename, 'w', newline='') as csvfile:
    ...
    while cap.isOpened():
        ...
        if len(results[0].keypoints) >= 2:  # Ensure we have detected at least two people
            ...
            # Get wrist and head heights
            wrist_height = min(ball_carrier[9][1], ball_carrier[10][1])
            head_height = tackler[0][1]

            # Write data to CSV
            csv_writer.writerow([frame_count, ball_carrier_speed, tackler_speed, impact_detected, head_below_wrists, head_back_hips_inline, wrist_height, head_height])
